...
function createStick(areaId, knobId, topic, autoCenterX, autoCenterY) {
    ...
    const resetKnob = () => {
        ...
        const normX = autoCenterX ? 0 : (x - originX) / (rect.width / 2 - knob.offsetWidth / 2);
        const normY = autoCenterY ? 0 : -(y - originY) / (rect.height / 2 - knob.offsetHeight / 2);
        sendJoystickInput(topic, normX, normY);
    };
...
function setupMouseEvents(leftStick, rightStick) {
    ...
    leftStick.movePointerToCenter = () => {
        ...
        leftStick.movePointer(rect.left + rect.width / 2, rect.top + rect.height / 2);
    };
    rightStick.movePointerToCenter = () => {
        const rect = rightStick.area.getBoundingClientRect();
        rightStick.movePointer(rect.left + rect.width / 2, rect.top + rect.height / 2);
    };
}
