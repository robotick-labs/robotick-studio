...
export function init() {
    ...
    // Use the token once it's loaded
    secretsPromise.then(() => {
...
function lookAtRocket(position) {
    ...
    const offset = Cesium.Matrix4.multiplyByPoint(transform, cameraOffset, new Cesium.Cartesian3());
...
async function updateRocketFromTelemetry() {
    ...
    const roll = Cesium.Math.toRadians(parseFloat(data["jsb.fcs_attitude_roll_deg"]));
    const pitch = Cesium.Math.toRadians(parseFloat(data["jsb.fcs_attitude_pitch_deg"]));
    const yaw = Cesium.Math.toRadians(parseFloat(data["jsb.fcs_attitude_yaw_deg"]));
