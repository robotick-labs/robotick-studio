...
#!/usr/bin/env bash
set -euo pipefail
# Wrapper generated by install.sh. It translates CLI flags (e.g. --smoke-test)
# into the environment variables that the Electron bootstrap expects before
# handing control to the packaged Electron binary.
TOOLKIT="$(cd "$(dirname "$0")/.." && pwd)"
args=()
while [[ $# -gt 0 ]]; do
  case "$1" in
    --smoke-test)
      export ROBOTICK_SMOKE_TEST=1
      shift
      ;;
    --)
      shift
...
exec "$TOOLKIT/node_modules/.bin/electron" "$TOOLKIT" "${args[@]}"
EOF
