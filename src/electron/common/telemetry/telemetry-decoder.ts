...
// Shared telemetry layout and raw-buffer decoder used by Electron and renderer adapters.
...
function readSingle(
  ...
  const toSafeNumber = (value: bigint): number | bigint => {
    const max = BigInt(Number.MAX_SAFE_INTEGER);
    const min = BigInt(Number.MIN_SAFE_INTEGER);
    return value <= max && value >= min ? Number(value) : value;
  };
  ...
  switch (type) {
    ...
    case "int64_t":
      return safeRead(8, () => toSafeNumber(view.getBigInt64(offset, true)));
    case "uint64_t":
      return safeRead(8, () => toSafeNumber(view.getBigUint64(offset, true)));
    case "uint16_t":
