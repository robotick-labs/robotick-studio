...
export interface WorkloadSpec { name: string; type?: string; }
...
export class ModelStore {
  ...
  load(models: { modelPath: string; data: ModelData }[]) {
    this.models.clear();
    for (const m of models) this.models.set(m.modelPath, structuredClone(m.data));
    this.version++;
  }

  getModelIds(): string[] { return [...this.models.keys()]; }
  get(modelId: string): ModelData | undefined { return this.models.get(modelId); }
  ...
  laneChildren(modelId: string, laneIndex: number): string[] {
    ...
    const lanes = root.type === "SyncedGroupWorkload"
      ? (root.children ?? [])
      : [root.name];
  ...
  private setLaneChildren(modelId: string, laneIndex: number, names: string[]) {
    const m = this.models.get(modelId)!;
    const root = m.workloads.find(w => w.name === m.root)!;
    const lanes = root.type === "SyncedGroupWorkload" ? (root.children ?? []) : [root.name];
    const parentName = lanes[laneIndex];
    const parent = m.workloads.find(w => w.name === parentName)!;
    if (!parent.children) {
  ...
  moveWithinLane(modelId: string, laneIndex: number, fromSlot: number, toSlot: number) {
  ...
  insertAt(modelId: string, laneIndex: number, slot: number, spec: WorkloadSpec) {
    const m = this.models.get(modelId)!;
  ...
  rename(modelId: string, oldName: string, next: string) {
    ...
    for (const ww of m.workloads) {
      if (ww.children) ww.children = ww.children.map(n => n === oldName ? next : n);
    }
