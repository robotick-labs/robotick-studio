...
def _remove_managed_custom_stage_script_if_needed(config: Config, script_path: Path) -> None:
    ...
    if config.dry_run:
        print(f"[grey]📝 Dry run — would remove stale custom stage script:[/] {script_path}")
        return
...
def _write_custom_stage_override_scripts_if_needed(config: Config) -> None:
    ...
    for stage_key, script_name in CUSTOM_STAGE_SCRIPT_NAMES.items():
        ...
        command_resolved = command_raw.replace("${PROJECT_DIR}", str(config.project_dir)).strip()
        script_contents = "\n".join(
            [
                "#!/bin/bash",
                "set -euo pipefail",
        ...
        if config.dry_run:
            print(f"[grey]📝 Dry run — would write custom stage script:[/] {script_path}")
            continue
