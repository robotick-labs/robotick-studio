...
| Floating panels | `src/renderer/components/workbenches/floating-panels/floating-panel-store.ts` | In-memory map of panel records scoped by workspace. |
...
| Workspace memory | `src/renderer/utils/workbenchMemory.ts` | Helper functions now backed by `services/storage`. |
