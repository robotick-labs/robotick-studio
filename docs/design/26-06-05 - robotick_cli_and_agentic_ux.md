...
- [ ] Promote Studio workbench/layout state to first-class project resources
      Deliverable: durable Studio workbench/layout/panel/viewer state stops living primarily in generic renderer storage and becomes explicit per-project, shared Studio resources with product-level names, structure, versioning, and atomic writes. `window-state.json` should shrink to true window/session chrome state only, while important workbench definitions, layout tabs, panel layouts, floating panels, and viewer defaults move into dedicated project-backed Studio resource files rather than an opaque `renderer-storage.json` bucket. Migration must automatically preserve existing user state by reading legacy storage keys and legacy on-disk renderer storage, including current `workspace`-named keys and any newly introduced `workbench` aliases, then writing the canonical project resource form without data loss. Prefer JSON rather than YAML here because Studio already uses YAML for static app config, while its runtime/persisted Electron-side state is already JSON-oriented (`window-state.json`, `renderer-storage.json`).
      Recommended Codex model/effort: `gpt-5.4` / `medium`

- [ ] Align implementation and CLI naming on `studio_workbench` / `workbench`
      Deliverable: hub contracts, Studio state/resource names, renderer routes, config structures, component names, persisted storage keys, implementation-facing docs, CLI command grammar, and CLI flags all use `studio_workbench`, `studio_workbench_id`, `workbench`, and `--workbench` for the Studio route/work-area concept. This is a Studio rename as well as a CLI/contract rename: current Studio-facing `workspace` terminology for work areas should move to `workbench`, while `robotick_workspace` remains the checkout/runtime-registration concept.
