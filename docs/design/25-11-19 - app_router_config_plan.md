...
## Goals
- Drive top-level workbenches and navigation menus from a single YAML config to enable per-project or per-user customization.
...
## Requirements
1. **Config files**: YAML under `src/renderer/config/app-workbenches.yaml` and `src/renderer/config/app-editors.yaml` (canonical locations). Each workbench entry in `app-workbenches.yaml` defines:
   - `id`: unique string.
...
2. **AppConfigService**:
   ...
   - Exposes immutable `workbenches` array and a `useAppConfig` hook.
...
4. **Testing**:
   ...
   - Simple integration test that Router renders a workbench defined in YAML.

## Deliverables
- `src/renderer/config/app-workbenches.yaml`
