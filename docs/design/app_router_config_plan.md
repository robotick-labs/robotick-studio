...
## Goals
- Drive top-level routes and navigation menus from a single YAML config to enable per-project or per-user customization.
...
## Requirements
1. **Config file**: YAML under `src/renderer/config/routes.yaml` (canonical location). Each entry defines:
   - `id`: unique string.
...
2. **AppConfigService**:
   ...
   - Exposes immutable `routes` array and a `useAppConfig` hook.
   - Future-friendly for feature flags/overrides.
3. **Router/Header updates**:
   - Router consumes `routes` to generate `<Route>` entries.
...
4. **Testing**:
   ...
   - Simple integration test that Router renders a route defined in YAML.
